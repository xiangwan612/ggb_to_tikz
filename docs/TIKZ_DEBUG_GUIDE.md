...
- `导出矢量图`：将最近一次成功编译的结果复制到剪切板，或保存为纯矢量 PDF。
- `AI优化`：预留入口（当前未接入智能改写）。
- `复制代码`：复制左侧当前 TikZ 代码。
- `关闭`：关闭调试器弹窗。
...
中间为双栏：
- 左栏：`TikZ 代码`（可直接编辑）
- 右栏：`编译预览`（本地 LaTeX 优先，TikZJax 回退）
...
标签和线/面微调采用防抖编译：连续点击或长按时只更新参数，停止操作后才编译最终结果，中间保留上一张成功预览。
...
### Q1：预览为空怎么办？
- 先查看预览区下方的编译状态；本地 LaTeX 错误会直接显示具体原因。
- 本地启动时会优先使用 `pdflatex` 生成高清预览，支持范围比 TikZJax 更完整。
- 若本机未安装 LaTeX 或后端不可用，页面会自动回退到 TikZJax，并显示回退状态。
